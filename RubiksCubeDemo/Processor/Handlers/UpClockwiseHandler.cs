...
namespace RubiksCubeDemo.Processor.Handlers
{
    internal class UpClockwiseHandler : RotationHandlerBase, IRotationHandler
    {
        ...
        protected override void MoveNeighbours()
        {
            ...
            rightFace.Cubies[1, 0] = backFaceCubiesCopy[1, 0];
            rightFace.Cubies[2, 0] = backFaceCubiesCopy[2, 0];

            frontFace.Cubies[0, 0] = rightFaceCubiesCopy[0, 0];
            frontFace.Cubies[1, 0] = rightFaceCubiesCopy[1, 0];
            frontFace.Cubies[2, 0] = rightFaceCubiesCopy[2, 0];

            leftFace.Cubies[0, 0] = frontFaceCubiesCopy[0, 0];
            leftFace.Cubies[1, 0] = frontFaceCubiesCopy[1, 0];
            leftFace.Cubies[2, 0] = frontFaceCubiesCopy[2, 0];

            backFace.Cubies[0, 0] = leftFaceCubiesCopy[0, 0];
            backFace.Cubies[1, 0] = leftFaceCubiesCopy[1, 0];
